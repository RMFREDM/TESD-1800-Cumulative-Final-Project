	...
	it("logs into accounts", () => {
		...
		cy.get('form[name="login-form"]')
			...
			.whithin(($form) => {
				...
				cy.get('form[label[for="email"]').should("be.visible");
				cy.get('form[input[name="email"][required][type="email"]')
					.should("be.visible")
					.type(email);

				cy.get('form[label[for="password"]').should("be.visible");
				cy.get('form[input[name="password"][required][type="password"]')
					.should("be.visible")
					.type(password);
